...
public class CssFileDAOTest extends BaseDAOTest {
    ...
    public void getContent_returnsCorrectIfEntryExists() {
        // set-up
        jdbcTemplate.execute("DELETE FROM roger.cssfile where cpinstanceid=-1");
        jdbcTemplate.execute("INSERT INTO roger.cssfile (id, cpinstanceid, content) "
    ...
    public void persistContent_insertWhenNoEntryExists() {
        ...
        int dbRowCount = jdbcTemplate.queryForObject("select count(1) from roger.cssfile where cpinstanceid=-1",
                Integer.class);
        String dbContent = jdbcTemplate.queryForObject("select content from roger.cssfile where cpinstanceid=-1",
                String.class);
