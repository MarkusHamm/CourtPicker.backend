...
public class CourtCategoryDAO {
    ...
    public CourtCategory get(int id) {
        String query = "select * from roger.courtcategory where id=?";
    ...
    public CourtCategory persist(CourtCategory courtCategory) {
        ...
        if (courtCategory.getId() == null) {
            int newRecordId = jdbcTemplate.queryForInt("select nextval('roger.courtcategory_id_seq')");
            courtCategory.setId(newRecordId);
            query = "insert into roger.courtcategory (cpinstanceid, ordernr, name, bookablefromtime, bookabletotime, " +
            		"bookingunit, id) values (?, ?, ?, ?, ?, ?, ?)";
        }
        // do an update if id is set
        else {
            query = "update roger.courtcategory set cpinstanceid=?, ordernr=?, name=?, bookablefromtime=?, " +
